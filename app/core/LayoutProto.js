...
export default class Layout extends React.Component {
    ...
    solidifyWidgets() {
        ...
        widgets.forEach(function (widget) {
            widget.actWidth = widget.tmpWidth == 0 ? widget.actWidth : widget.tmpWidth, widget.actHeight = widget.tmpHeight == 0 ? widget.actHeight : widget.tmpHeight, widget.tmpWidth = 0;
            widget.tmpHeight = 0;
    ...
    intersects(cRect, iRect, offset) {
        if ((cRect.left >= iRect.left + offset && cRect.left <= iRect.left + iRect.width - offset || iRect.left + offset >= cRect.left && iRect.left + offset <= cRect.left + cRect.width) && (cRect.top >= iRect.top + offset && cRect.top <= iRect.top + iRect.height - offset || iRect.top + offset >= cRect.top && iRect.top + offset <= cRect.top + cRect.height)) {
            return true;
        }
        return false;
    }
    ...
    collisionDetect(id) {
        ...
        var findPushDirection = function (id, cRect, iRect) {
            ...
            if (gauge.bottom == iRect.top + iRect_height) down = iRect_width;else if (gauge.top == iRect.top + iRect.height - iRect_height) up = iRect_width;
            if (gauge.left == iRect.left + iRect.width - iRect_width) left = iRect_height;else if (gauge.right == iRect.left + iRect_width) right = iRect_height;
    ...
    render() {
        ...
        var widgets = this.state.widgets.map(function (widget, i) {
            return React.createElement(Widget, { id: widget.id,
                key: widget.id,
        ...
        return React.createElement(
            ...
            { className: 'layout',
                ref: 'layoutRef' },
            React.createElement('div', { id: 'navBar', className: 'themeSecondaryColor' }),
            React.createElement(Grid, { gridCols: this.state.gridCols,

...
var Layout = function () {
    ...
    function isAvailableArea(element, target) {
        ...
        for (var i = 0; i < addedWidgets.length; i++) {
            ...
            if ((target.col >= iComp.col && target.col <= iComp.col + iComp.width || iComp.col >= target.col && iComp.col <= target.col + target.width) && (target.row >= iComp.row && target.row <= iComp.row + iComp.height || iComp.row >= target.row && iComp.row <= target.row + target.height) && addedWidgets[i].id != element.id) {
                return false;
            }
        }
    ...
    function findPushDirection(pRect, cRect) {
        ...
        if (iRect.bottom == cRect.top + iRect_height) down = iRect_width;else if (iRect.top == cRect.bottom - iRect_height) up = iRect_width;
        if (iRect.left == cRect.right - iRect_width) left = iRect_height;else if (iRect.right == cRect.left + iRect_width) right = iRect_height;
    ...
    function pointIntersections(a, b) {
        if ((a.col >= b.col && a.col <= b.col + b.width || b.col >= a.col && b.col <= a.col + a.width) && (a.row >= b.row && a.row <= b.row + b.height || b.row >= a.row && b.row <= a.row + a.height)) {
            return true;
        }
        return false;
    }
                ...
                if (iElement.id != collider.id && !iWidget.pushing) {
                    ...
                    if (pointIntersections(target, iCurrent)) {
                        ...
                        switch (push_direction) {
                            case 'up':
                                if (iCurrent.row > 0 && detectChainCollisions(iWidget, 0, dTop) && Math.abs(dTop) <= iCurrent.height + target.height + 1) {
                                    direction = "up " + dTop;
                                    if (iCurrent.row - dTop >= 0) y += (cellHeight + 5) * dTop;
                                    iWidget.pushedRow = dTop;
                                    pushed = true;
                                }
                                if (!pushed && iCurrent.row + iCurrent.height + 1 < gridRows && detectChainCollisions(iWidget, 0, dBot) && Math.abs(dBot) <= iCurrent.height + target.height + 1) {
                                    direction = "down " + dBot;
                            ...
                            case 'down':
                                ...
                                if (!pushed && iCurrent.row > 0 && detectChainCollisions(iWidget, 0, dTop) && Math.abs(dTop) <= iCurrent.height + target.height + 1) {
                                    direction = "up " + dTop;
                                    if (iCurrent.row - dTop >= 0) y += (cellHeight + 5) * dTop;
                                    iWidget.pushedRow = dTop;
                                    pushed = true;
                            ...
                            case 'left':
                                if (!pushed && iCurrent.col > 0 && detectChainCollisions(iWidget, dLeft, 0) && Math.abs(dLeft) <= iCurrent.width + target.width + 1) {
                                    direction = "left " + dLeft;
                                    if (iCurrent.col - dLeft >= 0) x += (cellWidth + 5) * dLeft;
                                    iWidget.pushedCol = dLeft;
                                    pushed = true;
                                }
                                if (!pushed && iCurrent.col + iCurrent.width + 1 < gridCols && detectChainCollisions(iWidget, dRight, 0) && Math.abs(dRight) <= iCurrent.width + target.width + 1) {
        ...
        freePushed: function () {
            ...
            for (var i = 0; i < addedWidgets.length; i++) {
                ...
                if (collider.id != iWidget.id && !iWidget.pushing) {
                    ...
                    if (iWidget.pushedCol == 0 && iWidget.pushedRow == 0) {
                        //reset pushed state
                        iWidget.pushed = false;
                    }
                }

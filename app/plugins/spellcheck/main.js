        ...
        for (var i = 0; i < words.length; i++) {
            if (words[i].length > 0 && words[i] != ' ') {
                if (dictionary.indexOf(words[i]) == -1) {
                    var error = pm.markRange(pos, pos + words[i].length, { className: 'misspellEditorText', removeWhenEmpty: true, onRemove: function () {
                            console.debug("REMOVED MISSPELL");
                        } });
                    errors.push(error);
                    console.debug("Misspelled:", words[i]);
                } else {
                    var mark = pm.markRange(pos, pos + words[i].length, { className: 'defaultEditorText', removeWhenEmpty: true, onRemove: function () {
                            console.debug("REMOVED MISSPELL");
                        } });

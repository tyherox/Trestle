...
var NotePane = React.createClass({
    ...
    moveNote: function (from, to) {
        ...
        notes.forEach(function (note) {
            if (neighbor) {
                ...
                if (note.drag) note.order = to;else if (note.order == from) {
                    if (from > to) note.order--;else note.order++;
                } else if (note.order == to) {
                    if (from < to) note.order--;else note.order++;
                } else if (note.order < from && note.order > to) note.order++;else if (note.order < to && note.order > from) note.order--;
            }
        });
        this.setState({ notes: notes });
    },
    ...
    render: function () {
        return React.createElement(
            'div',
            { id: 'id2' },
            React.createElement('input', { id: 'id2-inputField',
                ref: 'search',
                onChange: this.search,
                className: 'inputField themeTextColor themeSecondaryColor' }),
            React.createElement(
                'div',
                { id: 'id2-scrollPane', ref: 'scrollPane' },
...
var NoteList = React.createClass({
    ...
    render: function () {
        ...
        notes.forEach(function (note) {
            top = totalTop;
            if (!note.visible) totalTop += 0;else if (note.toggleState && !note.reorder) totalTop += note.height + 5;else totalTop += 30;
            note.top = top;
        });

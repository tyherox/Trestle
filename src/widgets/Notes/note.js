...
export default class Note extends React.Component {
    ...
    componentDidUpdate(){
        ...
        var self = this, note =  self.refs.note;

        if(!this.props.drag && this.props.visible) note.style.transform = 'translate(' + this.props.left + 'px, ' + this.props.top + 'px)';
        else if(!this.props.visible) note.style.transform = 'translate(' + this.props.left + 'px, ' + -this.props.height + 'px)';

        if(this.props.drag) note.style.boxShadow = '0 0 5px #536DFE';
        else note.style.boxShadow = "0 2px 2px 0 rgba(0, 0, 0, 0.16), 0 0px 2px 0 rgba(0, 0, 0, 0.12)";

        if(this.props.toggleState &&
            !this.props.reorder &&
            this.refs.editorContainer.parentNode != note){
        ...
        else if((!this.props.toggleState || this.props.reorder ) &&
            this.refs.editorContainer.parentNode === note){
    ...
    componentDidMount(){
        ...
        var xTarget, yTargetnote = this.refs.note, offs = note.offsetTop, refX = 0, refY = 0 ;
    ...
    render() {
        ...
        return(
            <div className = 'id2-note' ref = 'note' onClick = {this.focus} onDragOver={this.test}>
                <div className = 'id2-toolbar'>
                    <button className = 'widgetToolbarButtons id2-handler'
                            id = 'handler'
                            ref = 'handle'/>
                    <input className = 'id2-title'
                           ref = 'title'
                ...
                <div className = 'id2-editor' ref = 'editorContainer'>
